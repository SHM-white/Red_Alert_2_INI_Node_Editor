...
void IBRF_Bump::IBR_SendToF(const IBR_ToFMessage& Msg)
{
    RFStack.Push(Msg);
    RFNewInfo.store(true);
}
void IBRF_Bump::IBF_SendToR(const IBF_ToRMessage& Msg)
{
    FRStack.Push(Msg);
}
...
{
    FRLock.lock();
    std::vector<IBF_ToRMessage> DStack = FRDelayStack.GetCopyAndClear();
    std::vector<uint64_t> DTime = FRDelay.GetCopyAndClear();
    FRLock.unlock();
    ...
    FRLock.lock();
    std::vector<IBF_ToRMessage> DStackA = FRDelayStack.GetCopyAndClear();
    std::vector<uint64_t> DTimeA = FRDelay.GetCopyAndClear();
...
void IBRF_Bump::IBF_ForceProc()
{
    if (IBF_TimeToTake())
    {
        for (auto Msg : IBF_TakeAway())Msg.Action();
    }
}
...
{
    RFLock.lock();
    std::vector<IBR_ToFMessage> DStack = RFDelayStack.GetCopyAndClear();
    std::vector<uint64_t> DTime = RFDelay.GetCopyAndClear();
    RFLock.unlock();
    ...
    RFLock.lock();
    std::vector<IBR_ToFMessage> DStackA = RFDelayStack.GetCopyAndClear();
    std::vector<uint64_t> DTimeA = RFDelay.GetCopyAndClear();
    if (!DStackA.empty())for (auto v : DStackA)DStack.push_back(v);
    if (!DTimeA.empty())for (auto v : DTimeA)DTime.push_back(v);
